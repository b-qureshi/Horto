 ...
 */
package com.example.horto.common.opengl;
...
public class Mesh implements Closeable {
   ...
   * (location 2, vec3).
   */
  public static <Obj> Mesh createFromAsset(Render render, String assetFileName) throws IOException {
    try (InputStream inputStream = render.getAssets().open(assetFileName)) {
      Obj obj = (Obj) ObjUtils.convertToRenderable(ObjReader.read(inputStream));

      // Obtain the data from the OBJ, as direct buffers:
      IntBuffer vertexIndices = ObjData.getFaceVertexIndices((ReadableObj) obj, /*numVerticesPerFace=*/ 3);
      FloatBuffer localCoordinates = ObjData.getVertices((ReadableObj) obj);
      FloatBuffer textureCoordinates = ObjData.getTexCoords((ReadableObj) obj, /*dimensions=*/ 2);
      FloatBuffer normals = ObjData.getNormals((ReadableObj) obj);
